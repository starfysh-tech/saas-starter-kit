...
const handlePOST = async (req: NextApiRequest, res: NextApiResponse) => {
  return Sentry.startSpan(
    ...
    async (span) => {
      ...
      span.setAttribute('hasInviteToken', !!inviteToken);
      span.setAttribute('hasTeam', !!team);
      ...
      const user = await Sentry.startSpan(
        ...
        async (userSpan) => {
          userSpan.setAttribute('email', email);
          userSpan.setAttribute('hasInvitation', !!invitation);
      ...
      if (!invitation) {
        userTeam = await Sentry.startSpan(
          {
            op: 'db.create',
            name: 'Create Team',
          },
          async (teamSpan) => {
            teamSpan.setAttribute('teamName', team);
            teamSpan.setAttribute('userId', user.id);
            teamSpan.setAttribute('slug', slugify(team));
      ...
      span.setAttribute('teamId', userTeam?.id || '');
      span.setAttribute(
        'requiresEmailConfirmation',
        env.confirmEmail && !user.emailVerified
      );
